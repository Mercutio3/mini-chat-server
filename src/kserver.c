...
// Client wants to disconnect
              ...
              // Private message
              processMsgCmd(event_fd, connectedClients, message,
                            USERNAME_MAX_LENGTH);
            } else {
              // Unknown command
              const char unknownCmdMsg[] = "Unknown command.";
              if (send(event_fd, unknownCmdMsg, strlen(unknownCmdMsg), 0) ==
                  -1) {
                perror("send error");
              }
            }
...
// Regular message; echo to all clients (and optionally to sender)
            ...
            struct clientNode *current = connectedClients;
            LOG_INFO("Broadcasting message from %s to other clients...",
                     senderName);
            char fullMessage[MAXDATASIZE + USERNAME_MAX_LENGTH + 4]; // Extra
            snprintf(fullMessage, sizeof(fullMessage), "%s: %s", senderName,
                     message);

            while (current != NULL) {
              if (current->fd != event_fd) {
                if (send(current->fd, fullMessage, strlen(fullMessage), 0) ==
                    -1) {
                  perror("send error");
                ...
                LOG_DEBUG("Sent message '%s' to Client %d.", message,
                          current->fd);
              }
