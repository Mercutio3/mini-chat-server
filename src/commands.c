...
void processNameCmd(int client_fd, struct clientNode *head, const char *newName,
                    int maxLength) {
  // Change username
  // Username can't be empty or longer than max length
  if (strlen(newName) == 0 || (size_t)strlen(newName) >= (size_t)maxLength - 1 ||
      strchr(newName, ' ') != NULL) {
    const char invalidNameMsg[] =
        "Invalid username. Please choose a different one.";
    ...
    if (nameTaken) {
      ...
      LOG_INFO("Client with fd %d changed their name to %s.", client_fd,
               newName);
...
void processMsgCmd(int client_fd, struct clientNode *head, char *message,
                   int maxLength) {
  ...
  if (target == NULL || privMsg == NULL) {
    const char invalidFormat[] =
        "Invalid format. Use /msg <username> <message>";
    ...
    if (targetFound == 0) {
      LOG_INFO("%s tried to send a private message to non-existent user %s.",
               senderName, target);
      // If not found
      ...
      snprintf(fullMsg, sizeof(fullMsg), "Private message from %s: %s",
               senderName, privMsg);
